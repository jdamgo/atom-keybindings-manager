...
export function applyAllModifications(allModifications) {
  ...
  atom.keymaps.keyBindings.forEach(e => {
    ...
    if (!arr) {
      ...
      arr.findByModification = function(mod) {
        let _mod = {}
        ;["package", "selector", "command", "keystrokes", "source"].forEach(property => {
          _mod[property] = Array.isArray(mod[property]) ? mod[property][0] : mod[property] 
        })
        return this.filter(e =>
            e.selector === _mod.selector &&
            e.command === _mod.command &&
            e.keystrokes === _mod.keystrokes &&
            (_mod.package === CORE_PACKAGE_NAME ?
                e.source.startsWith(_mod.source) :
                fs.realpathSync(e.source).startsWith(_mod.source)))
      }
  ...
  if (catchedErrors.length)
    atom.notifications.addError("Skipped keybinding modifications where applying failed!", {
      detail: (() => {
        let text = ""
        catchedErrors.forEach(err => {
          text += `\n${err.message}\n  ...at entry ${err.queryParams.package}/${err.queryParams.command} using '${err.queryParams.keystrokes}'`
        })
        return text
      })(),
