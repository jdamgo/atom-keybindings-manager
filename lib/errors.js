...
export class UselessModificationError extends ExtendableError {
  static get ERRCODE_NO_CHANGE() { return "EBADMODIFICATION" }

  constructor(modification, errcode) {
    super()
    this.modification = modification
    this.code = errcode
    message: {
      switch (errcode) {
      case this.constructor.ERRCODE_NO_CHANGE:
        this.message = "Modification not changing anything!"
        break message
      default:
        throw new Error(`Unknown error code in constructor of '${this.name}'! (${errcode})`)
      }
