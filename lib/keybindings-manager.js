...
export default {
  ...
  testing() {
    let obj = require(require("path").join(atom.packages.resolvePackagePath("keybindings-manager"), "lib/keybinding-actions"))
    obj.validateAndNormalizeModification({
      action: "remove",
  ...
  activate(state) { // eslint-disable-line no-unused-vars
    ...
    this.subscriptions.add(atom.commands.add("atom-workspace", "keybindings-manager:toggle", this.testing))
