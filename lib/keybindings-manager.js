...
export default {
  ...
  init() {
    this.bindingsTable = {}
    atom.keymaps.keyBindings.forEach(e => { this.bindingsTable[e.command] = e })
  },
  ...
  processModifications(modifications) {
    for (let mod of modifications)
      this.processModification(mod, this.bindingsTable)
  ...
  activate(state) { // eslint-disable-line no-unused-vars
    ...
    this.subscriptions.add(atom.packages.onDidActivateInitialPackages(() => { this.init(); this.testing() }))
  },
