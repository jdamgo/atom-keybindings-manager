...
import { ModificationParsingError, PackageNotAvailableError, UselessModificationError } from "./errors"
...
function _validateModification(mod) {
  let anyChangeValuePresent = false
  ...
  { /* *** validate package and set source (required) *** */
    ...
    anyChangeValuePresent = anyChangeValuePresent || (hasChangeValue && mod.package[0] !== mod.package[1])
  }
  ...
  { /* *** validate selector (required) *** */
    ...
    anyChangeValuePresent = anyChangeValuePresent || (hasChangeValue && mod.selector[0] !== mod.selector[1])
  }
  ...
  { /* *** validate command (required) *** */
    ...
    anyChangeValuePresent = anyChangeValuePresent || (hasChangeValue && mod.command[0] !== mod.command[1])
  }
  ...
  { /* *** validate & normalize keystrokes (required) *** */
    ...
    anyChangeValuePresent = anyChangeValuePresent || (hasChangeValue && mod.keystrokes[0] !== mod.keystrokes[1])
  }

  // check if change action without any changes
  if (mod.action === CHANGE_ACTION && !anyChangeValuePresent)
    throw new UselessModificationError(mod, UselessModificationError.ERRCODE_NO_CHANGE)
}
...
export function validateAllModifications(modifications) {
  ...
  modifications.forEach(mod => {
    try {
      ...
      else if (error instanceof UselessModificationError)
        if (!catchedErrors.has(UselessModificationError))
          catchedErrors.set(UselessModificationError, [error])
        else
          catchedErrors.get(UselessModificationError).push(error)
      else throw error
      mod.valid = false
    }
  })
  ...
  if (catchedErrors.has(ModificationParsingError)) {
    let notification = atom.notifications.addWarning("Skipped keybinding modifications where parsing failed!", {
      buttons: [{
      ...
      detail: (() => {
        ...
        catchedErrors.get(ModificationParsingError).forEach(err => {
          text += `\n${err.message}\n  ...at entry ${_modVal(err.modification.package)}/${_modVal(err.modification.command)} using '${_modVal(err.modification.keystrokes)}'`
        })
        return text
      })(),
  ...
  // process useless modifications errors (if any)
  /* ESLint Info
   *  curly: [..., "multi"]
   *  The rules autofix removes the curly braces of the if statement which leads
   *  to a syntax error, because `var`, `let`, `const` are not allowed for a
   ...
   */// eslint-disable-next-line curly
  if (catchedErrors.has(UselessModificationError)) {
    let notification = atom.notifications.addInfo("Skipped useless keybinding modifications!", {
      buttons: [{
        text: "Clean",
        // TODO .onDidClick - clean useless modifications
        onDidClick() {
          notification.dismiss()
        },
      }],
      detail: (() => {
        let text = ""
        catchedErrors.get(UselessModificationError).forEach(err => {
          text += `\n${err.message}\n  ...at entry ${_modVal(err.modification.package)}/${_modVal(err.modification.command)} using '${_modVal(err.modification.keystrokes)}'`
        })
        return text
      })(),
