...
class PropertyParsingError extends ExtendableError {
  constructor(errcode, modification, property, propertyValue) {
    ...
    default: throw new Error(PropertyParsingError, `Unknown error code!${errcode ? ` (${errcode})` : ""}`) // TODO FatalError
    }
    if (errcode !== ERRCODE_UNDEFINED)
...
export default function(mod) {
  ...
  if (!mod.action) throw new PropertyParsingError(ERRCODE_UNDEFINED, mod, "action")
  if (typeof mod.action !== "string") throw new PropertyParsingError(ERRCODE_TYPE, mod, "action", mod.action)
  // check if known action
  if (ALL_ACTIONS.indexOf(mod.action) < 0)
    throw new PropertyParsingError(ERRCODE_VALUE, mod, "action", mod.action)

  /* *** validate package and set source *** */
  // package property: required
  if (!mod.package) throw new PropertyParsingError(ERRCODE_UNDEFINED, mod, "package")
  if (typeof mod.package !== "string" && !(Array.isArray(mod.package) && mod.package.length === 2))
    throw new PropertyParsingError(ERRCODE_TYPE, mod, "package", mod.package)
  // validate query and change value (if available)
  for (let mpackage of (typeof mod.package !== "string" ? [mod.package] : mod.package)) {
    if (typeof mpackage !== "string") throw new PropertyParsingError(ERRCODE_TYPE, mod, "package", mpackage)
  ...
  if (!mod.selector) throw new PropertyParsingError(ERRCODE_UNDEFINED, mod, "selector")
  if (typeof mod.selector !== "string" && !(Array.isArray(mod.selector) && mod.selector.length === 2))
    throw new PropertyParsingError(ERRCODE_TYPE, mod, "selector", mod.selector)
  // validate query and change value (if available)
  for (let selector of (typeof mod.selector !== "string" ? [mod.selector] : mod.selector)) {
    if (typeof selector !== "string") throw new PropertyParsingError(ERRCODE_TYPE, mod, "selector", selector)
    // check if selector string has valid format
    if (!isSelectorValid(selector))
      throw new PropertyParsingError(ERRCODE_SYNTAX, mod, "selector", selector)
  }

  /* *** validate command *** */
  // command property: require
  if (!mod.command) throw new PropertyParsingError(ERRCODE_UNDEFINED, mod, "command")
  if (typeof mod.command !== "string" && !(Array.isArray(mod.command) && mod.command.length === 2))
    throw new PropertyParsingError(ERRCODE_TYPE, mod, "command", mod.command)
  // validate query and change value (if available)
  for (let command of (typeof mod.command !== "string" ? [mod.command] : mod.command)) {
    if (typeof command !== "string") throw new PropertyParsingError(ERRCODE_TYPE, mod, "command", command)
    ...
    if (!/[a-z-]+:[a-z-]+/.test(command) && command !== NATIVE_COMMAND_STRING)
      throw new PropertyParsingError(ERRCODE_SYNTAX, mod, "command", command)
    else if (command === NATIVE_COMMAND_STRING && mod.action === ADD_ACTION)
      throw new /* TODO error type? */ ModificationParsingError("trying to add a native! command", mod)
  }

  /* *** validate & normalize keystrokes *** */
  // keystrokes property: required
  if (!mod.keystrokes) throw new PropertyParsingError(ERRCODE_UNDEFINED, mod, "keystrokes")
  if (typeof mod.keystrokes !== "string" && !(Array.isArray(mod.keystrokes) && mod.keystrokes.length === 2))
    throw new PropertyParsingError(ERRCODE_TYPE, mod, "keystrokes", mod.keystrokes)
  // validate query and change value (if available)
  for (let keystrokes of (typeof mod.keystrokes !== "string" ? [mod.keystrokes] : mod.keystrokes)) {
    if (typeof keystrokes !== "string") throw new PropertyParsingError(ERRCODE_TYPE, mod, "keystrokes", keystrokes)
    // check if normalization was successful
    keystrokes = normalizeKeystrokes(keystrokes)
    if (!keystrokes)
      throw new PropertyParsingError(ERRCODE_SYNTAX, mod, "keystrokes", keystrokes)
  }
  ...
  if (typeof mod.priority !== "number" && !(Array.isArray(mod.priority) && mod.priority.length === 2))
    throw new PropertyParsingError(ERRCODE_TYPE, mod, "priority", mod.priority)
  // validate query and change value (if available)
  for (let priority of (typeof mod.priority !== "number" ? [mod.priority] : mod.priority)) {
    if (typeof priority !== "number") throw new PropertyParsingError(ERRCODE_TYPE, mod, "priority", priority)
    // check if priority is a positive integer or zero
    if (priority < 0 || priority % 1)
      throw new PropertyParsingError(ERRCODE_VALUE, mod, "priority", priority)
  }
